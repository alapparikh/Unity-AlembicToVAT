...
public class ATV_Editor : EditorWindow
{
    ...
    public bool writeColorTexture = false;
    public bool writeToPNG = false;
    ...
    void OnGUI()
    {
        ...
        writeColorTexture = EditorGUILayout.Toggle("Export color texture", writeColorTexture);
        GUILayout.Space(2);
    ...
    IEnumerator ExportFrames()
    {
        ...
        if ((MeshToBake != null) && (MeshToBake.childCount != 0))
        {
            ...
            if (VariableTopology == TopologyType.Variable)
            {
                ...
                // ALAP - assume hasColors is true if using Quill
                if (writeColorTexture)
                {
                    hasColors = true;
                }
            }
            ...
            if (VariableTopology == TopologyType.Variable)
            {
                for (int i = 0; i < verticesCount; i++) // everything is initialized to 0
                {
                    ...
                    // ALAP - Initialize colors to white
                    if (writeColorTexture)
                    {
            ...
            if (exportVAT)
            {
                ...
                Texture2D colorTexture = new Texture2D(textureWidth, textureHeight, TextureFormat.RGBAHalf, true, true);
                if (VariableTopology == TopologyType.Variable)
                ...
                for (int frame = 0; frame < framesCount; frame++)
                {
                    ...
                    if (VariableTopology == TopologyType.Variable)
                    {
                        ...
                        if (localMeshFilter != null)
                        {
                            ...
                            List<Color> local_colors = new List<Color>();
                            localMeshFilter.sharedMesh.GetColors(local_colors);
                            ...
                            for (int targetIndex = 0; targetIndex < maxTriangleCount * 3; targetIndex++)
                            {
                                ...
                                Color newVertexColor = Color.white;
                                ...
                                if (targetIndex < local_index.Length)
                                {
                                    ...
                                    newVertexColor = local_colors[vtxIndex];
                                }
                                ...
                                if (writeColorTexture)
                                    colorTexture.SetPixel(coordinates.x, coordinates.y, newVertexColor);
                            }
                ...
                if (writeColorTexture)
                {
                    Debug.Log("Saving colors texture asset at " + finalExportPath + ExportFilename + "_color.asset");
                    AssetDatabase.CreateAsset(colorTexture, finalExportPath + ExportFilename + "_color.asset");
                    AssetDatabase.SaveAssets();
                }
